...
use XML::Twig;
use Win32::GUI qw(WS_CAPTION WS_SIZEBOX WS_EX_CONTROLPARENT WS_CHILD DS_CONTROL WS_VISIBLE WS_VSCROLL WS_TABSTOP WS_CLIPCHILDREN);
	...
	my $coderef = eval "{
		...
		sub {
			foreach (\@{\$self->{_worder_}{$name}}) {
				if (\$_ ne '$name') {
					my \$width = isdigit(\$self->{_width_}{$name}{\$_}) ?
						\$self->{_width_}{$name}{\$_} :
						eval \$self->{_width_}{$name}{\$_};
			...
			foreach (\@{\$self->{_horder_}{$name}}) {
				if (\$_ ne '$name') {
					my \$height = isdigit(\$self->{_height_}{$name}{\$_}) ?
						\$self->{_height_}{$name}{\$_} :
						eval \$self->{_height_}{$name}{\$_};
			...
			foreach (\@{\$self->{_lorder_}{$name}}) {
				if (\$_ ne '$name') {
					my \$left = isdigit(\$self->{_left_}{$name}{\$_}) ?
						\$self->{_left_}{$name}{\$_} :
						eval \$self->{_left_}{$name}{\$_};
			...
			foreach (\@{\$self->{_torder_}{$name}}) {
				if (\$_ ne '$name') {
					my \$top = isdigit(\$self->{_top_}{$name}{\$_}) ?
						\$self->{_top_}{$name}{\$_} :
						eval \$self->{_top_}{$name}{\$_};
...
Win32::GUI::XMLBuilder will autogenerate a name for a widget if a 'name' attribute is not
...
sub WGXPost {
	my ($self, $t, $e) = @_;
	if ((caller(2))[0] ne 'XML::Parser::Expat') {
		$self->debug("Skipping child WGXPost element.");
		return;
	}
	$self->debug($e->text);
	my $ret = eval "{ package main; no strict; ".$e->text."}";
...
sub _GenericTop {
	...
	$e->{'att'}->{'pushstyle'} = WS_CLIPCHILDREN();
...
sub WGXPanel {
	...
	$e->{'att'}->{'pushstyle'} = WS_CHILD()|DS_CONTROL()|WS_VISIBLE()|WS_CLIPCHILDREN();
...
sub TabStrip {
	...
	$e->{'att'}->{'onChange'} = "sub {
		...
		for (\$i = 0; \$i < \$_[0]->Count; \$i++) {
			\$self->{\${\$self->{$name}->{Tabs}}[\$i]}->Show(\$_[0]->SelectedItem == \$i ? 1 : 0);
		}
	}";
	...
	@{$self->{$name}->{Tabs}} = (); # holds tabs
	@{$self->{$name}->{WGXStack}} = ($self, $t, $e, $name, $parent); # stash this for later
	foreach my $item ($e->children()) {
		$self->{$name}->InsertTab(-item => $item);
	}
}

=item InsertTab(%options)

Inserts a new tab into a TabStrip object. By providing a 'wgxdata' attribute one can control
child window construction.

=cut

sub Win32::GUI::TabStrip::InsertTab {
	my ($tabstrip, %options) = @_;
	
	my ($self, $t, $e, $name, $parent) = @{$tabstrip->{WGXStack}};
	
	my $item;
	if (!exists $options{'-item'}) {
		$item = XML::Twig::Elt->parse("<Item name='$options{-name}'>$options{-wgxdata}</Item>");
		foreach my $o (keys %options) {
			next if $o eq '-wgxdata';
			$o =~ s/^-//;
			$item->{'att'}->{$o} = $options{"-$o"};
		}
		$item->paste('last_child' => $e);
	}
	else {
		$item = $options{'-item'};
	}
	...
	my $TabIndex = $self->{$name}->InsertItem($self->evalhash($item));
	splice(@{$self->{$name}->{Tabs}}, $TabIndex, 0, $bname); # add tab to list

	my $f;
	$f->{'att'}->{'parent'} = $self->{$parent};
	$f->{'att'}->{'popstyle'} = WS_CAPTION()|WS_SIZEBOX()|WS_EX_CONTROLPARENT();
	$f->{'att'}->{'pushstyle'} = WS_CHILD()|DS_CONTROL()|WS_CLIPCHILDREN();
	$f->{'att'}->{'pushstyle'} |= WS_VISIBLE() if $TabIndex == 0;
	...
	$self->{$name}->{$TabIndex} = $bname; # stash index to name mapping!
	$self->{$bname}->{TabIndex} = $TabIndex;
	...
	&{$self->{$bname}->GetEvent('Resize')};
	return $self->{$bname};
}

=item DeleteTab($index, [$newindex])

Deletes the tab $index from the TabStrip object. Selects the tab to the left or first tab
afterwards unless $newindex is specified.

=cut

sub Win32::GUI::TabStrip::DeleteTab {
	my ($tabstrip, $tabindex, $newindex) = @_;
	
	my ($self, $t, $e, $name, $parent) = @{$tabstrip->{WGXStack}};
	
	# remove the item from tab strip
	# then DESTROY the panel below
	#
	my $bname = splice(@{$self->{$name}->{Tabs}}, $tabindex, 1); # remove tab from list
	$tabstrip->DeleteItem($tabindex);
	$self->{$bname}->DESTROY;
	delete $self->{$bname};

	if ($tabstrip->Count > 0) {
		my $selectindex = defined $newindex ?  
			$newindex : 
			$tabindex > 1 ? 
				$tabindex - 1 : 
				0;
	
		$tabstrip->SelectTab($selectindex);
	}
}

=item SelectTab($index)

Selects the tab $index from the TabStrip object.

=cut

sub Win32::GUI::TabStrip::SelectTab {
	my ($tabstrip, $tabindex) = @_;
	
	my ($self, $t, $e, $name, $parent) = @{$tabstrip->{WGXStack}};
	
	# select the item from tab strip
	# then Show the panel below
	#
	$tabstrip->Select($tabindex);
	&{$tabstrip->GetEvent('Change')};
}

=item GetIndexByName($name)

Returns the tab's index given its name. Returns undef if cannot find name.

=cut

sub Win32::GUI::TabStrip::GetIndexByName {
	my ($tabstrip, $bname) = @_;
	
	my ($self, $t, $e, $name, $parent) = @{$tabstrip->{WGXStack}};
	
	foreach my $index (0..$#{$self->{$name}->{Tabs}}) {
		if (${$self->{$name}->{Tabs}}[$index] eq $bname) {
			return $index;
		}
	}
